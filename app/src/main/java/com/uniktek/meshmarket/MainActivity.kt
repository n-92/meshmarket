...
import com.uniktek.meshmarket.onboarding.BluetoothCheckScreen
import com.uniktek.meshmarket.onboarding.BluetoothStatus
import com.uniktek.meshmarket.onboarding.BluetoothStatusManager
import com.uniktek.meshmarket.onboarding.BatteryOptimizationManager
import com.uniktek.meshmarket.onboarding.BatteryOptimizationPreferenceManager
import com.uniktek.meshmarket.onboarding.BatteryOptimizationScreen
import com.uniktek.meshmarket.onboarding.BatteryOptimizationStatus
import com.uniktek.meshmarket.onboarding.BackgroundLocationPermissionScreen
import com.uniktek.meshmarket.onboarding.InitializationErrorScreen
import com.uniktek.meshmarket.onboarding.InitializingScreen
import com.uniktek.meshmarket.onboarding.LocationCheckScreen
import com.uniktek.meshmarket.onboarding.LocationStatus
import com.uniktek.meshmarket.onboarding.LocationStatusManager
import com.uniktek.meshmarket.onboarding.OnboardingCoordinator
import com.uniktek.meshmarket.onboarding.OnboardingState
import com.uniktek.meshmarket.onboarding.PermissionExplanationScreen
import com.uniktek.meshmarket.onboarding.PermissionManager
import com.uniktek.meshmarket.ui.ChatScreen
import com.uniktek.meshmarket.ui.ChatViewModel
import com.uniktek.meshmarket.ui.OrientationAwareActivity
import com.uniktek.meshmarket.ui.theme.MeshMarketTheme
import com.uniktek.meshmarket.nostr.PoWPreferenceManager
...
class MainActivity : OrientationAwareActivity() {
    ...
    private val forceFinishReceiver = object : android.content.BroadcastReceiver() {
        override fun onReceive(context: android.content.Context, intent: android.content.Intent) {
            if (intent.action == com.uniktek.meshmarket.util.AppConstants.UI.ACTION_FORCE_FINISH) {
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        val filter = android.content.IntentFilter(com.uniktek.meshmarket.util.AppConstants.UI.ACTION_FORCE_FINISH)
        if (android.os.Build.VERSION.SDK_INT >= 33) {
            registerReceiver(
                forceFinishReceiver,
                filter,
                com.uniktek.meshmarket.util.AppConstants.UI.PERMISSION_FORCE_FINISH,
                null,
        ...
        com.uniktek.meshmarket.service.AppShutdownCoordinator.cancelPendingShutdown()
        ...
        try { com.uniktek.meshmarket.service.MeshForegroundService.start(applicationContext) } catch (_: Exception) { }
        meshService = com.uniktek.meshmarket.service.MeshServiceHolder.getOrCreate(applicationContext)
        ...
        lifecycleScope.launch {
            ...
            if (permissionManager.isFirstTimeLaunch()) {
                ...
                if (permissionManager.needsBackgroundLocationPermission() &&
                    ...
                    !com.uniktek.meshmarket.onboarding.BackgroundLocationPreferenceManager.isSkipped(this@MainActivity)
                ) {
                    mainViewModel.updateOnboardingState(OnboardingState.BACKGROUND_LOCATION_EXPLANATION)
                } else {
    ...
    private fun initializeApp() {
        ...
        lifecycleScope.launch {
            try {
                ...
                com.uniktek.meshmarket.nostr.LocationNotesInitializer.initialize(this@MainActivity)
    ...
    private fun handleNotificationIntent(intent: Intent) {
        val shouldOpenPrivateChat = intent.getBooleanExtra(
            com.uniktek.meshmarket.ui.NotificationManager.EXTRA_OPEN_PRIVATE_CHAT, 
            false
        )
        
        val shouldOpenGeohashChat = intent.getBooleanExtra(
            com.uniktek.meshmarket.ui.NotificationManager.EXTRA_OPEN_GEOHASH_CHAT,
            false
        )
        
        when {
            shouldOpenPrivateChat -> {
                val peerID = intent.getStringExtra(com.uniktek.meshmarket.ui.NotificationManager.EXTRA_PEER_ID)
                val senderNickname = intent.getStringExtra(com.uniktek.meshmarket.ui.NotificationManager.EXTRA_SENDER_NICKNAME)
            ...
            shouldOpenGeohashChat -> {
                val geohash = intent.getStringExtra(com.uniktek.meshmarket.ui.NotificationManager.EXTRA_GEOHASH)
                ...
                if (geohash != null) {
                    ...
                    val level = when (geohash.length) {
                        7 -> com.uniktek.meshmarket.geohash.GeohashChannelLevel.BLOCK
                        6 -> com.uniktek.meshmarket.geohash.GeohashChannelLevel.NEIGHBORHOOD
                        5 -> com.uniktek.meshmarket.geohash.GeohashChannelLevel.CITY
                        4 -> com.uniktek.meshmarket.geohash.GeohashChannelLevel.PROVINCE
                        2 -> com.uniktek.meshmarket.geohash.GeohashChannelLevel.REGION
                        else -> com.uniktek.meshmarket.geohash.GeohashChannelLevel.CITY // Default fallback
                    }
                    val geohashChannel = com.uniktek.meshmarket.geohash.GeohashChannel(level, geohash)
                    val channelId = com.uniktek.meshmarket.geohash.ChannelID.Location(geohashChannel)
