...
class BluetoothConnectionManager(
    ...
    init {
        ...
        try {
            val dbg = com.uniktek.meshmarket.ui.debug.DebugSettingsManager.getInstance()
            // Role enable/disable
    ...
    fun startServices(): Boolean {
        ...
        // }
            ...
            connectionScope.launch {
                ...
                val dbg = try { com.uniktek.meshmarket.ui.debug.DebugSettingsManager.getInstance() } catch (_: Exception) { null }
    ...
    /**
     * Force restart BLE scanning and advertising from scratch.
     * Clears stale registration state that can cause SCAN_FAILED_APPLICATION_REGISTRATION_FAILED.
     */
    fun forceRestart() {
        connectionScope.launch {
            Log.i(TAG, "Force restarting BLE client + server")
            clientManager.stop()
            serverManager.stop()
            delay(500)
            // Force performance mode for immediate discovery
            powerManager.forcePerformance()
            if (isActive) {
                serverManager.start()
                clientManager.start()
            }
            Log.i(TAG, "Force restart complete")
        }
    }
    ...
    override fun onPowerModeChanged(newMode: PowerManager.PowerMode) {
        ...
        connectionScope.launch {
            ...
            val serverEnabled = try { com.uniktek.meshmarket.ui.debug.DebugSettingsManager.getInstance().gattServerEnabled.value } catch (_: Exception) { true }
            if (serverEnabled) {
            ...
            if (wasUsingDutyCycle != nowUsingDutyCycle) {
                ...
                val clientEnabled = try { com.uniktek.meshmarket.ui.debug.DebugSettingsManager.getInstance().gattClientEnabled.value } catch (_: Exception) { true }
                if (clientEnabled) {
