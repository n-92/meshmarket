...
class SecurityManager(private val encryptionService: EncryptionService, private val myPeerID: String) {
    ...
    companion object {
        ...
        private const val MESSAGE_TIMEOUT = com.uniktek.meshmarket.util.AppConstants.Security.MESSAGE_TIMEOUT_MS // 5 minutes (same as iOS)
        private const val CLEANUP_INTERVAL = com.uniktek.meshmarket.util.AppConstants.Security.CLEANUP_INTERVAL_MS // 5 minutes
        private const val MAX_PROCESSED_MESSAGES = com.uniktek.meshmarket.util.AppConstants.Security.MAX_PROCESSED_MESSAGES
        private const val MAX_PROCESSED_KEY_EXCHANGES = com.uniktek.meshmarket.util.AppConstants.Security.MAX_PROCESSED_KEY_EXCHANGES
    }
    ...
    fun validatePacket(packet: BitchatPacket, peerID: String): Boolean {
        ...
        if (processedMessages.contains(messageID)) {
            ...
            val isFreshAnnounce = messageType == MessageType.ANNOUNCE &&
                    packet.ttl >= com.uniktek.meshmarket.util.AppConstants.MESSAGE_TTL_HOPS
    ...
    private fun verifyPacketSignature(packet: BitchatPacket, peerID: String): Boolean {
        try {
            // only verify ANNOUNCE, MESSAGE, FILE_TRANSFER, and LISTING
            ...
            if (MessageType.fromValue(packet.type) == MessageType.ANNOUNCE) {
                ...
                try {
                    val announcement = com.uniktek.meshmarket.model.IdentityAnnouncement.decode(packet.payload)
