package com.uniktek.meshmarket.mesh

import android.content.Context
import android.util.Log
import com.uniktek.meshmarket.crypto.EncryptionService
import com.uniktek.meshmarket.model.BitchatMessage
import com.uniktek.meshmarket.protocol.MessagePadding
import com.uniktek.meshmarket.model.RoutedPacket
import com.uniktek.meshmarket.model.IdentityAnnouncement
import com.uniktek.meshmarket.model.NoisePayload
import com.uniktek.meshmarket.model.NoisePayloadType
import com.uniktek.meshmarket.protocol.BitchatPacket
import com.uniktek.meshmarket.protocol.MessageType
import com.uniktek.meshmarket.protocol.SpecialRecipients
import com.uniktek.meshmarket.model.RequestSyncPacket
import com.uniktek.meshmarket.sync.GossipSyncManager
import com.uniktek.meshmarket.util.toHexString
...
class BluetoothMeshService(private val context: Context) {
    ...
    companion object {
        ...
        private val MAX_TTL: UByte = com.uniktek.meshmarket.util.AppConstants.MESSAGE_TTL_HOPS
    }
    ...
    private val serviceNotificationManager = com.uniktek.meshmarket.ui.NotificationManager(
        context.applicationContext,
        androidx.core.app.NotificationManagerCompat.from(context.applicationContext),
        com.uniktek.meshmarket.util.NotificationIntervalManager()
    )
        ...
        gossipSyncManager = GossipSyncManager(
            ...
            configProvider = object : GossipSyncManager.ConfigProvider {
                override fun seenCapacity(): Int = try {
                    com.uniktek.meshmarket.ui.debug.DebugPreferenceManager.getSeenPacketCapacity(500)
                } catch (_: Exception) { 500 }

                override fun gcsMaxBytes(): Int = try {
                    com.uniktek.meshmarket.ui.debug.DebugPreferenceManager.getGcsMaxFilterBytes(400)
                } catch (_: Exception) { 400 }

                override fun gcsTargetFpr(): Double = try {
                    com.uniktek.meshmarket.ui.debug.DebugPreferenceManager.getGcsFprPercent(1.0) / 100.0
    ...
    private fun setupDelegates() {
        ...
        peerManager.delegate = object : PeerManagerDelegate {
            override fun onPeerListUpdated(peerIDs: List<String>) {
                ...
                try { com.uniktek.meshmarket.services.AppStateStore.setPeers(peerIDs) } catch (_: Exception) { }
            ...
            override fun onPeerRemoved(peerID: String) {
                ...
                try { com.uniktek.meshmarket.services.meshgraph.MeshGraphService.getInstance().removePeer(peerID) } catch (_: Exception) { }
        ...
        messageHandler.delegate = object : MessageHandlerDelegate {
            ...
            override fun updatePeerIDBinding(newPeerID: String, nickname: String,
                ...
                try {
                    com.uniktek.meshmarket.favorites.FavoritesPersistenceService.shared.findNostrPubkey(publicKey)?.let { npub ->
                        com.uniktek.meshmarket.favorites.FavoritesPersistenceService.shared.updateNostrPublicKeyForPeerID(newPeerID, npub)
                    }
                } catch (_: Exception) { }
                ...
                try {
                    when {
                        message.isPrivate -> {
                            ...
                            if (peer.isNotEmpty()) com.uniktek.meshmarket.services.AppStateStore.addPrivateMessage(peer, message)
                        }
                        message.channel != null -> {
                            com.uniktek.meshmarket.services.AppStateStore.addChannelMessage(message.channel!!, message)
                        }
                        else -> {
                            com.uniktek.meshmarket.services.AppStateStore.addPublicMessage(message)
                        }
                    }
                } catch (_: Exception) { }
                ...
                if (delegate == null && message.isPrivate) {
                    try {
                        ...
                        if (senderPeerID != null) {
                            ...
                            val preview = com.uniktek.meshmarket.ui.NotificationTextUtils.buildPrivateMessagePreview(message)
        ...
        packetProcessor.delegate = object : PacketProcessorDelegate {
            ...
            override fun handleAnnounce(routed: RoutedPacket) {
                serviceScope.launch {
                    ...
                    if (deviceAddress != null && pid != null) {
                        ...
                        val isDirect = routed.packet.ttl == com.uniktek.meshmarket.util.AppConstants.MESSAGE_TTL_HOPS
            ...
            override fun onListingReceived(listing: com.uniktek.meshmarket.model.ListingItem, fromPeerID: String) {
                delegate?.didReceiveListing(listing, fromPeerID)
            }
        }
        ...
        override fun onPacketReceived(packet: BitchatPacket, peerID: String, device: android.bluetooth.BluetoothDevice?) {
            ...
            try {
                com.uniktek.meshmarket.ui.debug.DebugSettingsManager.getInstance().logIncoming(
                    packet = packet,
                    fromPeerID = peerID,
                    fromNickname = null,
            ...
            override fun onDeviceConnected(device: android.bluetooth.BluetoothDevice) {
                ...
                try {
                    ...
                    com.uniktek.meshmarket.ui.debug.DebugSettingsManager.getInstance()
    ...
    /**
     * Broadcast a marketplace listing to the mesh.
     */
    fun broadcastListing(listing: com.uniktek.meshmarket.model.ListingItem) {
        serviceScope.launch {
            try {
                val payload = listing.encode()
                val packet = BitchatPacket(
                    type = MessageType.LISTING.value,
                    ttl = MAX_TTL,
                    senderID = myPeerID,
                    payload = payload
                )
                ...
                connectionManager.broadcastPacket(RoutedPacket(signedPacket))
                Log.d(TAG, "Broadcast listing: ${listing.title} (${payload.size} bytes)")
            } catch (e: Exception) {
                Log.e(TAG, "Failed to broadcast listing: ${e.message}")
            }
        }
    }
     ...
     */
    fun sendFileBroadcast(file: com.uniktek.meshmarket.model.BitchatFilePacket) {
        try {
     ...
     */
    fun sendFilePrivate(recipientPeerID: String, file: com.uniktek.meshmarket.model.BitchatFilePacket) {
        try {
            ...
            serviceScope.launch {
                ...
                if (encryptionService.hasEstablishedSession(recipientPeerID)) {
                    try {
                        ...
                        val noisePayload = com.uniktek.meshmarket.model.NoisePayload(
                            type = com.uniktek.meshmarket.model.NoisePayloadType.FILE_TRANSFER,
                            data = filePayload
                        )
                        ...
                        val packet = BitchatPacket(
                            ...
                            payload = encrypted,
                            signature = null,
                            ttl = com.uniktek.meshmarket.util.AppConstants.MESSAGE_TTL_HOPS
                        )
        ...
        serviceScope.launch {
            ...
            if (encryptionService.hasEstablishedSession(recipientPeerID)) {
                try {
                    ...
                    val privateMessage = com.uniktek.meshmarket.model.PrivateMessagePacket(
                    ...
                    val messagePayload = com.uniktek.meshmarket.model.NoisePayload(
                        type = com.uniktek.meshmarket.model.NoisePayloadType.PRIVATE_MESSAGE,
                        data = tlvData
                    )
    ...
    fun sendReadReceipt(messageID: String, recipientPeerID: String, readerNickname: String) {
        serviceScope.launch {
            ...
            val geo = runCatching { com.uniktek.meshmarket.services.MessageRouter.tryGetInstance() }.getOrNull()
            val isGeoAlias = try {
                val map = com.uniktek.meshmarket.nostr.GeohashAliasRegistry.snapshot()
                map.containsKey(recipientPeerID)
            } catch (_: Exception) { false }
            if (isGeoAlias && geo != null) {
                geo.sendReadReceipt(com.uniktek.meshmarket.model.ReadReceipt(messageID), recipientPeerID)
                return@launch
            }

            try {
                // Avoid duplicate read receipts: check persistent store first
                val seenStore = try { com.uniktek.meshmarket.services.SeenMessageStore.getInstance(context.applicationContext) } catch (_: Exception) { null }
                ...
                val readReceiptPayload = com.uniktek.meshmarket.model.NoisePayload(
                    type = com.uniktek.meshmarket.model.NoisePayloadType.READ_RECEIPT,
                ...
                val packet = BitchatPacket(
                    ...
                    payload = encrypted,
                    signature = null,
                    ttl = com.uniktek.meshmarket.util.AppConstants.MESSAGE_TTL_HOPS // Same TTL as iOS messageTTL
                )
    ...
    fun sendBroadcastAnnounce() {
        ...
        serviceScope.launch {
            val nickname = try { com.uniktek.meshmarket.services.NicknameProvider.getNickname(context, myPeerID) } catch (_: Exception) { myPeerID }
            ...
            try {
                ...
                if (directPeers.isNotEmpty()) {
                    val gossip = com.uniktek.meshmarket.services.meshgraph.GossipTLV.encodeNeighbors(directPeers)
                    tlvPayload = tlvPayload + gossip
                }
                // Always update our own node in the mesh graph with the neighbor list we used
                try {
                    com.uniktek.meshmarket.services.meshgraph.MeshGraphService.getInstance()
     ...
     */
    fun getSessionState(peerID: String): com.uniktek.meshmarket.noise.NoiseSession.NoiseSessionState {
        ...
        return try {
            ...
            val withRoute = try {
                ...
                if (rec != null && !rec.contentEquals(SpecialRecipients.BROADCAST)) {
                    ...
                    val path = com.uniktek.meshmarket.services.meshgraph.RoutePlanner.shortestPath(myPeerID, dest)
...
interface BluetoothMeshDelegate {
    ...
    fun didReceiveListing(listing: com.uniktek.meshmarket.model.ListingItem, fromPeerID: String)
