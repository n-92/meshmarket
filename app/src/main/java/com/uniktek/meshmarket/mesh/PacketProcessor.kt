...
class PacketProcessor(private val myPeerID: String) {
    ...
    private suspend fun handleReceivedPacket(routed: RoutedPacket) {
        ...
        when (messageType) {
            ...
            MessageType.LISTING -> handleListing(routed)
    ...
    /**
     * Handle marketplace listing broadcast
     */
    private suspend fun handleListing(routed: RoutedPacket) {
        val peerID = routed.peerID ?: "unknown"
        Log.d(TAG, "Processing listing from ${formatPeerForLog(peerID)}")
        val listing = com.uniktek.meshmarket.model.ListingItem.decode(routed.packet.payload)
        if (listing != null) {
            delegate?.onListingReceived(listing, peerID)
        } else {
            Log.w(TAG, "Failed to decode listing from ${formatPeerForLog(peerID)}")
        }
    }
...
interface PacketProcessorDelegate {
    ...
    fun onListingReceived(listing: com.uniktek.meshmarket.model.ListingItem, fromPeerID: String)
