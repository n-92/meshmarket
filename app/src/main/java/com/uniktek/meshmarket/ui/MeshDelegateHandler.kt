...
import com.uniktek.meshmarket.ui.NotificationTextUtils
...
class MeshDelegateHandler(
                ...
                if (state.getJoinedChannelsValue().contains(message.channel)) {
                    ...
                    val viewingGeohash = try {
                        if (channel.startsWith("geo:")) {
                            ...
                            selected is com.uniktek.meshmarket.geohash.ChannelID.Location && selected.channel.geohash.equals(geo, ignoreCase = true)
                        } else false
    ...
    override fun didUpdatePeerList(peers: List<String>) {
        coroutineScope.launch {
            ...
            runCatching { com.uniktek.meshmarket.services.MessageRouter.tryGetInstance()?.onPeersUpdated(peers) }
                ...
                if (isNostrAlias || isNoiseHex) {
                    ...
                    val canonical = com.uniktek.meshmarket.services.ConversationAliasResolver.resolveCanonicalPeerID(
                        ...
                        nostrPubHexForAlias = { alias ->
                            ...
                            if (com.uniktek.meshmarket.nostr.GeohashAliasRegistry.contains(alias)) {
                                com.uniktek.meshmarket.nostr.GeohashAliasRegistry.get(alias)
                            } else {
                                // Best-effort: derive pub hex from favorites mapping for mesh nostr_ aliases
                                val prefix = alias.removePrefix("nostr_")
                                val favs = try { com.uniktek.meshmarket.favorites.FavoritesPersistenceService.shared.getOurFavorites() } catch (_: Exception) { emptyList() }
                                favs.firstNotNullOfOrNull { rel ->
                                    rel.peerNostrPublicKey?.let { s ->
                                        runCatching { com.uniktek.meshmarket.nostr.Bech32.decode(s) }.getOrNull()?.let { dec ->
                    ...
                    if (canonical != currentPeer) {
                        ...
                        com.uniktek.meshmarket.services.ConversationAliasResolver.unifyChatsIntoPeer(state, canonical, listOf(currentPeer))
                    ...
                    val favoriteRel = try {
                        ...
                        if (noiseKey != null) {
                            com.uniktek.meshmarket.favorites.FavoritesPersistenceService.shared.getFavoriteStatus(noiseKey)
                        } else null
                    ...
                    if (favoriteRel?.isMutual == true) {
                        ...
                        if (noiseHex != currentPeer) {
                            com.uniktek.meshmarket.services.ConversationAliasResolver.unifyChatsIntoPeer(
                                state = state,
            ...
            peers.forEach { pid ->
                try {
                    ...
                    val npub = com.uniktek.meshmarket.favorites.FavoritesPersistenceService.shared.findNostrPubkey(noiseKey)
                    val tempNostrKey: String? = try {
                        if (npub != null) {
                            val (hrp, data) = com.uniktek.meshmarket.nostr.Bech32.decode(npub)
    ...
    private fun unifyChatsIntoPeer(targetPeerID: String, keysToMerge: List<String>) {
        com.uniktek.meshmarket.services.ConversationAliasResolver.unifyChatsIntoPeer(state, targetPeerID, keysToMerge)
    }
    ...
    override fun didReceiveListing(listing: com.uniktek.meshmarket.model.ListingItem, fromPeerID: String) {
        // Forwarded to ChatViewModel via its own didReceiveListing override
    }
     ...
     */
    fun getPeerInfo(peerID: String): com.uniktek.meshmarket.mesh.PeerInfo? {
