...
import com.uniktek.meshmarket.model.BitchatFilePacket
...
class MediaSendingManager(
    ...
    companion object {
        ...
        private const val MAX_FILE_SIZE = com.uniktek.meshmarket.util.AppConstants.Media.MAX_FILE_SIZE_BYTES // 50MB limit
    }
    ...
    /**
     * Send a video file (already compressed by VideoUtils)
     */
    fun sendVideoNote(toPeerIDOrNull: String?, channelOrNull: String?, filePath: String) {
        try {
            Log.d(TAG, "Starting video send: $filePath")
            val file = java.io.File(filePath)
            if (!file.exists()) {
                Log.e(TAG, "File does not exist: $filePath")
                return
            ...
            if (toPeerIDOrNull != null) {
                sendPrivateFile(toPeerIDOrNull, filePacket, filePath, BitchatMessageType.Video)
            } else {
                sendPublicFile(channelOrNull, filePacket, filePath, BitchatMessageType.Video)
            }
        } catch (e: Exception) {
    ...
    fun sendFileNote(toPeerIDOrNull: String?, channelOrNull: String?, filePath: String) {
        try {
            ...
            val mimeType = try { 
                com.uniktek.meshmarket.features.file.FileUtils.getMimeTypeFromExtension(file.name) 
            } catch (_: Exception) { 
            ...
            val messageType = when {
                ...
                mimeType.lowercase().startsWith("video/") -> BitchatMessageType.Video
    ...
    private fun sendPrivateFile(
        ...
        messageManager.updateMessageDeliveryStatus(
            msg.id,
            com.uniktek.meshmarket.model.DeliveryStatus.PartiallyDelivered(0, 100)
        )
     ...
     */
    fun handleTransferProgressEvent(evt: com.uniktek.meshmarket.mesh.TransferProgressEvent) {
        val msgId = synchronized(transferMessageMap) { transferMessageMap[evt.transferId] }
        if (msgId != null) {
            if (evt.completed) {
                messageManager.updateMessageDeliveryStatus(
                    msgId,
                    com.uniktek.meshmarket.model.DeliveryStatus.Delivered(to = "mesh", at = java.util.Date())
                )
                synchronized(transferMessageMap) {
                ...
                messageManager.updateMessageDeliveryStatus(
                    msgId,
                    com.uniktek.meshmarket.model.DeliveryStatus.PartiallyDelivered(evt.sent, evt.total)
                )
            }
        }
    }
