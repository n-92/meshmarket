...
import com.uniktek.meshmarket.favorites.FavoritesPersistenceService
import kotlinx.coroutines.flow.StateFlow
import kotlinx.coroutines.flow.MutableStateFlow
import kotlinx.coroutines.flow.asStateFlow
import com.uniktek.meshmarket.mesh.BluetoothMeshDelegate
import com.uniktek.meshmarket.mesh.BluetoothMeshService
import com.uniktek.meshmarket.service.MeshServiceHolder
import com.uniktek.meshmarket.model.BitchatMessage
import com.uniktek.meshmarket.model.BitchatMessageType
import com.uniktek.meshmarket.nostr.NostrIdentityBridge
import com.uniktek.meshmarket.protocol.BitchatPacket


import kotlinx.coroutines.launch
import com.uniktek.meshmarket.util.NotificationIntervalManager
import kotlinx.coroutines.delay
import kotlinx.coroutines.launch
import java.util.Date
import kotlin.random.Random
import com.uniktek.meshmarket.services.VerificationService
import com.uniktek.meshmarket.identity.SecureIdentityStateManager
import com.uniktek.meshmarket.noise.NoiseSession
import com.uniktek.meshmarket.nostr.GeohashAliasRegistry
import com.uniktek.meshmarket.util.dataFromHexString
import com.uniktek.meshmarket.util.hexEncodedString
import java.security.MessageDigest

/**
 * Refactored ChatViewModel - Main coordinator for MeshMarket functionality
...
class ChatViewModel(
    ...
    var meshService: BluetoothMeshService = initialMeshService
        private set
    private val debugManager by lazy { try { com.uniktek.meshmarket.ui.debug.DebugSettingsManager.getInstance() } catch (e: Exception) { null } }
    ...
    fun sendVideoNote(toPeerIDOrNull: String?, channelOrNull: String?, filePath: String) {
        mediaSendingManager.sendVideoNote(toPeerIDOrNull, channelOrNull, filePath)
    }
    ...
    val selectedLocationChannel: StateFlow<com.uniktek.meshmarket.geohash.ChannelID?> = state.selectedLocationChannel
    ...
    init {
        ...
        viewModelScope.launch {
            try { com.uniktek.meshmarket.services.AppStateStore.peers.collect { peers ->
                state.setConnectedPeers(peers)
                state.setIsConnected(peers.isNotEmpty())
            } } catch (_: Exception) { }
        }
        viewModelScope.launch {
            try { com.uniktek.meshmarket.services.AppStateStore.publicMessages.collect { msgs ->
                // Source of truth is AppStateStore; replace to avoid duplicate keys in LazyColumn
                state.setMessages(msgs)
            } } catch (_: Exception) { }
        }
        viewModelScope.launch {
            try { com.uniktek.meshmarket.services.AppStateStore.privateMessages.collect { byPeer ->
                ...
                try {
                    val seen = com.uniktek.meshmarket.services.SeenMessageStore.getInstance(getApplication())
        ...
        viewModelScope.launch {
            try { com.uniktek.meshmarket.services.AppStateStore.channelMessages.collect { byChannel ->
                // Replace with store snapshot
        ...
        viewModelScope.launch {
            com.uniktek.meshmarket.mesh.TransferProgressManager.events.collect { evt ->
    ...
    private fun loadAndInitialize() {
        ...
        viewModelScope.launch {
            com.uniktek.meshmarket.ui.debug.DebugSettingsManager.getInstance().debugMessages.collect { msgs ->
                if (com.uniktek.meshmarket.ui.debug.DebugSettingsManager.getInstance().verboseLoggingEnabled.value) {
                    ...
                    if (selectedLocation is com.uniktek.meshmarket.geohash.ChannelID.Mesh) {
        ...
        com.uniktek.meshmarket.favorites.FavoritesPersistenceService.initialize(getApplication())
        ...
        try {
            val nostrTransport = com.uniktek.meshmarket.nostr.NostrTransport.getInstance(getApplication())
    ...
    /**
     * Try to set nickname, rejecting if a connected peer already uses it.
     * Returns true if accepted, false if taken.
     */
    fun trySetNickname(newNickname: String): Boolean {
        val peerNicknames = meshService.getPeerNicknames()
        val taken = peerNicknames.any { (_, nick) ->
            nick.equals(newNickname, ignoreCase = true)
        }
    ...
    private fun ensureGeohashDMSubscriptionIfNeeded(convKey: String) {
        try {
            ...
            val repo = repoField.get(geohashViewModel) as com.uniktek.meshmarket.nostr.GeohashRepository
            ...
            if (!gh.isNullOrEmpty()) {
                ...
                val subMgr = subMgrField.get(geohashViewModel) as com.uniktek.meshmarket.nostr.NostrSubscriptionManager
                val identity = com.uniktek.meshmarket.nostr.NostrIdentityBridge.deriveIdentity(gh, getApplication())
                val subId = "geo-dm-$gh"
                ...
                if (currentId != subId) {
                    ...
                    subMgr.subscribeGiftWraps(
                        ...
                        handler = { event ->
                            ...
                            val dmHandler = dmHandlerField.get(geohashViewModel) as com.uniktek.meshmarket.nostr.NostrDirectMessageHandler
    ...
    fun openLatestUnreadPrivateChat() {
        try {
            ...
            val openPeer: String = if (targetKey.startsWith("nostr_")) {
                ...
                val canonical = com.uniktek.meshmarket.services.ConversationAliasResolver.resolveCanonicalPeerID(
                    ...
                    nostrPubHexForAlias = { alias -> com.uniktek.meshmarket.nostr.GeohashAliasRegistry.get(alias) },
                    findNoiseKeyForNostr = { key -> com.uniktek.meshmarket.favorites.FavoritesPersistenceService.shared.findNoiseKey(key) }
                )
                canonical ?: targetKey
            }
    ...
    fun sendMessage(content: String) {
        ...
        if (content.startsWith("/")) {
            ...
            commandProcessor.processCommand(content, meshService, meshService.myPeerID, { messageContent, mentions, channel ->
                if (selectedLocationForCommand is com.uniktek.meshmarket.geohash.ChannelID.Location) {
        ...
        if (selectedPeer != null) {
            ...
            selectedPeer = com.uniktek.meshmarket.services.ConversationAliasResolver.resolveCanonicalPeerID(
            ...
            privateChatManager.sendPrivateMessage(
                ...
                val router = com.uniktek.meshmarket.services.MessageRouter.getInstance(getApplication(), meshService)
                router.sendPrivate(messageContent, peerID, recipientNicknameParam, messageId)
            }
        } else {
            // Check if we're in a location channel
            val selectedLocationChannel = state.selectedLocationChannel.value
            if (selectedLocationChannel is com.uniktek.meshmarket.geohash.ChannelID.Location) {
    ...
    fun toggleFavorite(peerID: String) {
        ...
        try {
            ...
            if (peerInfo?.noisePublicKey != null) {
                ...
                if (peerID.length == 64 && peerID.matches(Regex("^[0-9a-fA-F]+$"))) {
                    try {
                        ...
                        val rel = com.uniktek.meshmarket.favorites.FavoritesPersistenceService.shared.getFavoriteStatus(noiseKey!!)
            ...
            if (noiseKey != null) {
                ...
                val identityManager = com.uniktek.meshmarket.identity.SecureIdentityStateManager(getApplication())
                ...
                com.uniktek.meshmarket.favorites.FavoritesPersistenceService.shared.updateFavoriteStatus(
                ...
                try {
                    val myNostr = com.uniktek.meshmarket.nostr.NostrIdentityBridge.getCurrentNostrIdentity(getApplication())
    ...
    private fun updateReactiveStates() {
        ...
        sessionStates.forEach { (peerID, newState) ->
            ...
            if (old != "established" && newState == "established") {
                com.uniktek.meshmarket.services.MessageRouter
    ...
    override fun didUpdatePeerList(peers: List<String>) {
        ...
        // Broadcast our listings to new peers
        if (peers.isNotEmpty() && dataManager.myListings.isNotEmpty()) {
            broadcastMyListings()
        }
    }
    ...
    override fun didReceiveListing(listing: com.uniktek.meshmarket.model.ListingItem, fromPeerID: String) {
        Log.d(TAG, "Received listing '${listing.title}' from ${fromPeerID.take(8)}, thumbs=${listing.thumbnails.size}")
        // Save thumbnails to disk so UI can display them
        if (listing.thumbnails.isNotEmpty()) {
            viewModelScope.launch(kotlinx.coroutines.Dispatchers.IO) {
                val dir = java.io.File(getApplication<android.app.Application>().filesDir, "listings/thumbs").apply { mkdirs() }
                val savedPaths = listing.thumbnails.mapIndexed { idx, thumbBytes ->
                    try {
                        val file = java.io.File(dir, "${listing.id}_$idx.jpg")
                        file.writeBytes(thumbBytes)
                        file.absolutePath
                    } catch (_: Exception) { "" }
                }
                val withPaths = listing.copy(imagePaths = savedPaths)
                state.addPeerListing(fromPeerID, withPaths)
            }
        } else {
            state.addPeerListing(fromPeerID, listing)
        }
    }
    ...
    fun panicClearAllData() {
        ...
        try {
            com.uniktek.meshmarket.services.SeenMessageStore.getInstance(getApplication()).clear()
        } catch (_: Exception) { }
        ...
        com.uniktek.meshmarket.features.file.FileUtils.clearAllMedia(getApplication())
        ...
        try {
            ...
            try {
                val store = com.uniktek.meshmarket.geohash.GeohashBookmarksStore.getInstance(getApplication())
                store.clearAll()
            } catch (_: Exception) { }
    ...
    fun selectLocationChannel(channel: com.uniktek.meshmarket.geohash.ChannelID) {
    ...
    val peerListings: StateFlow<Map<String, List<com.uniktek.meshmarket.model.ListingItem>>> = state.peerListings
    val showProfileSheet: StateFlow<String?> = state.showProfileSheet
    val showMyListingsSheet: StateFlow<Boolean> = state.showMyListingsSheet

    fun showProfileSheet(peerID: String) { state.setShowProfileSheet(peerID) }
    fun hideProfileSheet() { state.setShowProfileSheet(null) }
    fun showMyListingsSheet() { state.setShowMyListingsSheet(true) }
    fun hideMyListingsSheet() { state.setShowMyListingsSheet(false) }

    fun getMyListings(): List<com.uniktek.meshmarket.model.ListingItem> = dataManager.myListings

    fun addListing(listing: com.uniktek.meshmarket.model.ListingItem) {
        val withSeller = listing.copy(
            sellerPeerID = meshService.myPeerID,
            sellerNickname = state.nickname.value
        )
        dataManager.addListing(withSeller)
        // Generate thumbnails and broadcast
        viewModelScope.launch(kotlinx.coroutines.Dispatchers.IO) {
            val withThumbs = com.uniktek.meshmarket.model.ListingItem.withThumbnails(withSeller)
            meshService.broadcastListing(withThumbs)
        }
    }

    fun updateListing(listing: com.uniktek.meshmarket.model.ListingItem) {
        dataManager.updateListing(listing)
        viewModelScope.launch(kotlinx.coroutines.Dispatchers.IO) {
            val withThumbs = com.uniktek.meshmarket.model.ListingItem.withThumbnails(listing)
            meshService.broadcastListing(withThumbs)
        }
    }
    ...
    /** Broadcast all my listings to the mesh (e.g. on connect) */
    fun broadcastMyListings() {
        viewModelScope.launch(kotlinx.coroutines.Dispatchers.IO) {
            dataManager.myListings.forEach { listing ->
                val withThumbs = com.uniktek.meshmarket.model.ListingItem.withThumbnails(listing)
                meshService.broadcastListing(withThumbs)
            }
        }
    ...
    /**
     * Force refresh mesh: restart BLE scanning, re-advertise, and re-announce.
     * Useful when peers aren't discovered due to duty cycle or background throttling.
     */
    fun refreshMesh() {
        viewModelScope.launch(kotlinx.coroutines.Dispatchers.IO) {
            // Force restart BLE from scratch (fixes SCAN_FAILED_APPLICATION_REGISTRATION_FAILED)
            meshService.connectionManager.forceRestart()
            // Wait for scan/advertise to come back up
            delay(1500)
            // Re-send identity announcement to all connected peers
            meshService.sendBroadcastAnnounce()
            // Re-broadcast listings
            broadcastMyListings()
        }
    }
