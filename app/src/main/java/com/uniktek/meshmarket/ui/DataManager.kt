...
class DataManager(private val context: Context) {
    ...
    private val _myListings = mutableListOf<ListingItem>()
    val myListings: List<ListingItem> get() = _myListings.toList()

    fun loadListings() {
        try {
            val json = prefs.getString("my_listings", null) ?: return
            val type = object : TypeToken<List<ListingItem>>() {}.type
            val items: List<ListingItem> = gson.fromJson(json, type)
            _myListings.clear()
            _myListings.addAll(items)
            Log.d(TAG, "Loaded ${_myListings.size} listings")
        } catch (e: Exception) {
            Log.e(TAG, "Failed to load listings: ${e.message}")
        }
    }

    private fun saveListings() {
        prefs.edit().putString("my_listings", gson.toJson(_myListings)).apply()
    }

    fun addListing(listing: ListingItem) {
        _myListings.add(listing)
        saveListings()
    }

    fun updateListing(listing: ListingItem) {
        val idx = _myListings.indexOfFirst { it.id == listing.id }
        if (idx >= 0) {
            _myListings[idx] = listing
            saveListings()
        }
    }

    fun removeListing(listingId: String) {
        _myListings.removeAll { it.id == listingId }
        saveListings()
    }

    fun getListingById(id: String): ListingItem? = _myListings.find { it.id == id }
