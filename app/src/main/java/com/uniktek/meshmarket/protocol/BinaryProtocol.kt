package com.uniktek.meshmarket.protocol
...
enum class MessageType(val value: UByte) {
    ...
    LISTING(0x23u); // Marketplace listing broadcast
...
data class BitchatPacket(
    ...
    fun toBinaryDataForSigning(): ByteArray? {
        ...
        val unsignedPacket = BitchatPacket(
            ...
            route = route,
            ttl = com.uniktek.meshmarket.util.AppConstants.SYNC_TTL_HOPS // Use fixed TTL=0 for signing to ensure relay compatibility
        )
...
object BinaryProtocol {
    ...
    private fun decodeCore(raw: ByteArray): BitchatPacket? {
        try {
            ...
            if (payloadLength > com.uniktek.meshmarket.util.AppConstants.Protocol.MAX_PAYLOAD_LENGTH.toUInt()) {
                Log.w("BinaryProtocol", "Payload length ${payloadLength} exceeds maximum allowed (${com.uniktek.meshmarket.util.AppConstants.Protocol.MAX_PAYLOAD_LENGTH})")
                return null
            }
